...
/** Per-row approver note for specs, handed to Claude with the spec. */
    ...
    <div v-if="adminSubTab === 'triage'" class="nge-admin-section">
      <div class="nge-admin-block">
        ...
        <div v-for="row in triageRows" :key="row.id" class="nge-triage-card">
          ...
          <div v-if="row.approver_note" class="nge-triage-note"><span class="nge-triage-spec-label">Note</span> {{ row.approver_note }}</div>
          ...
          <textarea
            v-if="row.status === 'proposed' && isBuildable(row)"
            v-model="triageNotes[row.id]"
            class="nge-triage-message"
            rows="2"
            placeholder="Optional note for Claude, sent with the spec when you approve"
            @keydown.stop @keyup.stop @keypress.stop
          ></textarea>
